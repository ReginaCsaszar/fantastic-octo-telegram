$('#residentModal').on('show.bs.modal', function (event) {
    ...
    for (let i = 0; i < apis.length; i++) {
        $.getJSON(apis[i], function(response){
            ...
            $("table").append("<tr class='temp'><td>" + response["name"] + "</td><td>" + response["height"] + "</td><td>" + response["mass"] + "</td><td>" + response["skin_color"] + "</td><td>" + response["hair_color"] + "</td><td>" + response["eye_color"] + "</td><td>" + response["birth_year"] + "</td><td>" + response["gender"] + "</td></tr>");
        });
    }  
    var modal = $(this)
